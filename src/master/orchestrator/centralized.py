...
class CentralizedOrchestrator(BaseOrchestrator):
    ...
    def _prepare_data(self, payload: dict, base_seed: int):
        ...
        print(
            f"[{self.orchestrator_name}] ETL completato in "
            f"{etl_time:.2f}s. Dimensione: {df_clean.shape}"
        )
        ...
        # ---------------------------------------------------------
        # SALVATAGGIO TRAIN SET PER I WORKER
        # ---------------------------------------------------------
    ...
    def _execute_training_step(
        ...
        if self.current_job_id != job_id:
            ...
            self._prepare_data(payload, base_seed=seed)

        total_step_trees = target_alberi - start_alberi

        print(
            f"\n[{self.orchestrator_name}] Distribuzione carico: "
            f"{total_step_trees} alberi da generare (seed: {seed})..."
        )
        ...
        def _rpc_call(worker_name, worker_info, n_trees, worker_seed):
            print(
                f"[RPC -> {worker_name}] Invio richiesta per {n_trees} alberi "
                f"su {worker_info['host']}:{worker_info['port']}..."
            )
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for future in as_completed(future_to_worker):
                ...
                try:
                    ...
                    print(
                        f"[RPC <- {worker_name}] Ricevuti con successo "
                        f"{len(result_trees)} alberi."
                    )

                except Exception as e:
                    print(
                        f"[ERRORE CRITICO] Il worker '{worker_name}' "
                        f"ha fallito o si è disconnesso: {e}"
                    )
                    raise e
