...
class FederatedWorker(BaseWorker):
    ...
    def __init__(
        ...
        # Validation fold usato SOLO per la calibrazione della soglia federata
        # (vedi _load_and_preprocess_real_shard / exposed_get_validation_predictions).
        # None finché non viene popolato dal primo training di un job.
    ...
    def _load_and_preprocess_real_shard(self, worker_index: int, hyperparameters: dict, dataset_type: str = "real"):
        ...
        # Validation fold "pulito" (mai visto in training né usato come test
        # finale) da cui deriveremo più sotto self._cached_X_val/_y_val, usati
        # da FederatedOrchestrator._calibrate_federated_threshold per calibrare
        # decision_threshold sul modello federato reale (voto pesato per
        # foglia) invece di riusare quello calibrato sulla baseline
        # centralizzata. None finché non viene popolato più sotto (rimane None
        # per la regressione e per gli shard senza classe minoritaria/troppo
        # esigua: quei worker sono semplicemente esclusi dal pool di
        # calibrazione, vedi exposed_get_validation_predictions).
        df_val = None
        if not self.is_regression():
            ...
            if n_minority == 0:
                ...
                try:
                    ...
                    df_train_clean, df_val = validation_splitter.split(df_train_clean)
                    ...
                    print(f"[{self.worker_name}] [ATTENZIONE] Classe minoritaria presente ma troppo "
                          ...
                          f"com'è (probabile scarsa informatività degli alberi risultanti). Nessun "
                          f"validation set per la calibrazione della soglia da questo worker.")
        ...
        # Validation fold per la calibrazione della soglia federata (vedi
        # df_val più sopra): proiettato sullo STESSO spazio di feature di
        # train/test (selected_features), altrimenti il modello non potrebbe
        # scorarlo. None per la regressione e per i worker senza (o con
        # troppo poca) classe minoritaria: quei worker vengono semplicemente
        # esclusi dal pool di calibrazione lato Orchestratore.
        if df_val is not None:
            X_val_df = df_val[features_to_keep] if selected_features is not None else df_val.drop(columns=[self.target_column])
            self._cached_X_val = X_val_df.to_numpy(dtype=np.float64)
            self._cached_y_val = df_val[self.target_column].to_numpy(dtype=np.int64)
        else:
            self._cached_X_val = None
            self._cached_y_val = None
        ...
        print(f"[{self.worker_name}] Shard caricato in cache. X_train Shape: {self._cached_X_train.shape}"
              + (f", X_val Shape: {self._cached_X_val.shape}" if self._cached_X_val is not None else ", nessun validation set."))
